...
void Model::Draw(bool isMaterialUse)
{
...
void Model::Load( const char *FileName )
{
	...
	// 頂点バッファ生成
	{
		...
		for (unsigned int i = 0; i < model.VertexNum; i++)
		{
			// 頂点座標を記録する
			m_VertexPosition.push_back(model.VertexArray[i].Position);

			// 最小のサイズを求める
			if (MinSize.x > model.VertexArray[i].Position.x)
			{
				MinSize.x = model.VertexArray[i].Position.x;
			}
			if (MinSize.y > model.VertexArray[i].Position.y)
			{
				MinSize.y = model.VertexArray[i].Position.y;
			}
			if (MinSize.z > model.VertexArray[i].Position.z)
			{
				MinSize.z = model.VertexArray[i].Position.z;
			}
			// 最大のサイズを求める
			if (MaxSize.x < model.VertexArray[i].Position.x)
			{
				MaxSize.x = model.VertexArray[i].Position.x;
			}
			if (MaxSize.y < model.VertexArray[i].Position.y)
			{
				MaxSize.y = model.VertexArray[i].Position.y;
			}
			if (MaxSize.z < model.VertexArray[i].Position.z)
			{
				MaxSize.z = model.VertexArray[i].Position.z;
			}
		}

		m_ModelSize = MaxSize - MinSize;
